...
export class GLMService implements IAIService {
   ...
   */
  private async callAPI(messages: unknown[], retries = this.config.maxRetries, modelOverride?: string, maxTokensOverride?: number): Promise<Result<string>> {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        const payload: GLMRequestPayload = {
          ...
          top_p: 0.7,
          max_tokens: maxTokensOverride || 4096,  // Ensure enough tokens for complete response
        };
  ...
  async extractCriteria(fileId: string, pdfContent: string): Promise<Result<ExtractCriteriaResult>> {
    try {
      ...
      const result = await this.callAPI(messages, this.config.maxRetries, undefined, PDF_CONFIG.BATCH_AI_MAX_TOKENS);
  ...
  async recognizeMedications(fileId: string, content: string): Promise<Result<Array<{
    ...
    try {
      ...
      const result = await this.callAPI(messages, this.config.maxRetries, undefined, 4096);
  ...
  async extractSubjectNumber(fileId: string, content: string): Promise<Result<ExtractSubjectDataResult>> {
    try {
      ...
      const result = await this.callAPI(messages, this.config.maxRetries, undefined, 2048);

      if (!result.success) {
        console.error('[GLMService] extractSubjectNumber API call failed:', result.error?.message);
        return result;
      }

      const parseResult = PromptEngine.parseJSONResponse<ExtractSubjectDataResult>(result.data);
      if (!parseResult.success) {
        console.error('[GLMService] extractSubjectNumber JSON parse failed:', parseResult.error?.message);
        return parseResult;
      }

      return ok(parseResult.data);
    } catch (error) {
      console.error('[GLMService] extractSubjectNumber exception:', error);
      return err(createAppError(
        ErrorCode.AI_API_ERROR,
